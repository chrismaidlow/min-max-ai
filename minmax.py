...
class minmax_alphabeta(object):
    ...
    def minmax(self, game=None, maximizingPlayer=True):
        ...
        if maximizingPlayer:
            ...
            for child_game in game.children():
                bestPlay, value = self.minmax(child_game, False)

                if value > best_max:
                    best_move = bestPlay
                    best_max = value

            best_move.append(game.board)
            game.move(best_move.ii, best_move.jj)
            return bestPlay, best_max
        else:

            for child_game in game.children():
                bestPlay, value = self.minmax(child_game, True)
